...
class Calculator(object):
    ...
    def menus(self):
        ...
        fm =tkinter.Menu(allmenu,tearoff = 0)
        fm.add_command(label='标准型(T)\tAlt+1',command=self.myfunc)
        fm.add_command(label='科学型(S)\tAlt+2',command=self.myfunc)
        fm.add_command(label='程序员(P)\tAlt+3',command=self.myfunc)
        fm.add_command(label='统计信息(A)\tAlt+4',command=self.myfunc)
        fm.add_separator()
        fm.add_command(label='历史记录(Y)\tCtrl+H',command=self.myfunc)
        fm.add_command(label='数学分组(I)',command=self.myfunc)
        fm.add_separator()
        fm.add_command(label='基本(B)\tCtrl+F4',command=self.myfunc)
        fm.add_command(label='单位转换(U)\tCtrl+U',command=self.myfunc)
        fm.add_command(label='日期计算(D)\tCtrl+E',command=self.myfunc)
        self.root.config(menu=allmenu)

    def layout(self):
        result = tkinter.StringVar()
        result.set(0)
        show_label = tkinter.Label(self.root,bg='white',font=('宋体',30),anchor='e',textvariable=self.result)
        ...
        button_left = tkinter.Button(self.root,text='<-',command=self.dele_one)
        button_left.place(x=5,y=150,width=50,height=50)
        ...
        button_kpf = tkinter.Button(self.root,text='OK',command=self.kpf)
        ...
        button_7 = tkinter.Button(self.root,text='7',command=lambda: self.pressnum('9'))
        button_9.place(x=115,y=205,width=50,height=50)
        button_division = tkinter.Button(self.root,text='/',command=lambda: self.presscaculate('/'))
        button_division.place(x=170,y=205,width=50,height=50)
        button_remainder = tkinter.Button(self.root,text='//',command=lambda: self.presscaculate('//'))
        ...
        button_mutiplication = tkinter.Button(self.root,text='*',command=lambda: self.presscaculate('*'))
        button_7.place(x=170,y=260,width=50,height=50)
        ...
        button_1 = tkinter.Button(self.root,text='7',command=lambda: self.pressnum('1'))
        ...
        button_3.place(x=5,y=315,width=50,height=50)
        button_subtration = tkinter.Button(self.root,text='-',command=lambda: self.presscaculate('-'))
        button_subtration.place(x=170,y=315,width=50,height=50)
        button_equal = tkinter.Button(self.root,text='=',command=lambda: self.pressequal())
        button_equal.place(x=225,y=315,width=50,height=50)
        button_0 = tkinter.Button(self.root,text='0',command=lambda: self.pressnum('0'))
        button_0.place(x=5,y=370,width=50,height=50)
        button_point = tkinter.Button(self.root,text='.',command=lambda: self.pressnum('.'))
        button_point.place(x=115,y=370,width=50,height=50)
        button_plus = tkinter.Button(self.root,text='+',command=lambda: self.presscaculate('+'))
        button_7.place(x=170,y=370,width=50,height=50)

    def myfunc(self):
        tkinter.messagebox.showinfo()

    def pressnum(self,sum):
    ...
    def pressequal(self):
        ...
        calculator = ''.join(self.lists)
        endnum  = eval(calculator)
